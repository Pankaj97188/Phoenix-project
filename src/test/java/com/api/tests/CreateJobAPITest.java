...
public class CreateJobAPITest {
	...
	@Test(description = "Verify if the FD is able to create the Inwarranty Job!!!", groups = {"smoke","e2e","api"})
	public void createJobAPITest() {
		
	 jobId=	given()
			.header("Content-type", "application/json")
		.and()
			.header("Authorization", TestUtility.generateAuthToken(Roles.FD))
		.and()
			.body(convertPOJOToJSON(createJobPOJO))
		.when()
			.log().all()
			.post("job/create")
			.then()
